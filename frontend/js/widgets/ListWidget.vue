...
} from './voc/voc_dropdown_runtime.ts';
...
const dropdownRuntimeContext: VocDropdownRuntimeContext = {
  ...
  set _clickOutside(value) {
    clickOutsideHandler = value;
  },
  get _clickOutsideTimerId() {
    return clickOutsideTimerId;
  },
  set _clickOutsideTimerId(value) {
    clickOutsideTimerId = value;
  },
  get _scrollUpdate() {
    return scrollUpdate as (() => void) | null;
  },
...
function setHighlightedIndex(index: number, options: { scroll?: boolean } = {}): void {
  ...
  if (scroll && nextIndex >= 0) {
    void nextTick(() => scrollHighlightedItemIntoView(dropdownRuntimeContext));
  }
}
