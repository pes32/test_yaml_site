...
const RowRuntimeMethods = {
    ...
    duplicateTableRowRelative(rowIndex, where, anchorCol) {
        ...
        const cloneRow = Utils && Utils.cloneTableRowDeep;
        if (typeof cloneRow !== 'function') return;
        const row = this.normRow(rowIndex);
        const length = this.tableData.length;
        ...
        const copy = cloneRow(this.tableData[row], this.tableColumns);
        const nextLine =
            Utils && Utils.nextLineNumber
                ? Utils.nextLineNumber(this.tableData, this.tableColumns)
                : this.tableData.length + 1;
        if (Utils && typeof Utils.assignRowLineNumber === 'function') {
            Object.assign(copy, Utils.assignRowLineNumber(copy, this.tableColumns, nextLine));
        }
        if (where === 'above') {
            this.applyTableMutation(
                () => {
...
Core.RowRuntimeMethods = RowRuntimeMethods;
