...
const MenuRuntimeMethods = {
    ...
    computePasteAnchorRect(rect) {
        ...
        if (Ui && Ui.computePasteAnchorRect) {
            return Ui.computePasteAnchorRect(rect, this.selFocus);
        }
    ...
    clampMenuPosition(event) {
        const Ui = tableEngine.WidgetUiCoords;
        if (Ui && Ui.clampMenuPosition) return Ui.clampMenuPosition(event);
    ...
    onTableHeaderContextMenu(event, rowIndex, cell, colIndex) {
        ...
        const canGroup =
            Grouping &&
            typeof Grouping.canAddGroupingLevel === 'function' &&
            Grouping.canAddGroupingLevel(numCols, (this.groupingState.levels || []).length) &&
            colIndex >= 0 &&
            !isLineNumber;
...
Core.MenuRuntimeMethods = MenuRuntimeMethods;
