...
function normalizeIpLikeValue(rawValue: unknown, allowMask: boolean) {
...
function hasLiveIpLikeError(
    normalized: ReturnType<typeof normalizeIpLikeValue>,
    allowMask: boolean
): boolean {
...
function createIpLikeWidgetOptions(
    maskTemplate: string,
    validateFn: (value: string) => boolean,
    allowMask: boolean
) {
    const computed: WidgetComputedMap<IpLikeWidgetVm> = {
    ...
    const methods: WidgetMethodMap<IpLikeWidgetVm> = {
        emitValue() {
        ...
        applyNormalizedValue(rawValue, caretPosition) {
            ...
            void this.$nextTick(() => {
                const element = this.$el && 'querySelector' in this.$el
                    ? this.$el.querySelector('input.widget-ip')
                    : null;
                if (!(element instanceof HTMLInputElement)) return;
                if (element.value !== this.inputValue || this.inputValue === previousValue) return;
    ...
    return {
        ...
        emits: ['input'],
        setup(props: { widgetConfig: Record<string, unknown>; widgetName: string }, { emit }: { emit: (event: 'input', payload: unknown) => void }) {
        ...
        watch: {
            'widgetConfig.value': {
                immediate: true,
                handler(this: IpLikeWidgetVm, value: unknown) {
