...
- committed state page/modal-виджетов хранится централизованно в `frontend/js/runtime/page_session_store.js`;
- attrs YAML теперь проходит backend schema-validation, а `select_attrs` считается YAML-полем кнопки, не API-полем.
...
Что сейчас дополнительно проверяет backend:

- известный `widget` в attrs;
- допустимые ключи для каждого widget type;
- форма значений для high-risk attrs (`dialog`, `rows`, boolean flags, `source`, `select_attrs`).
