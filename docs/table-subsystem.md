...
- `TableWidget.vue` — Vue/UI orchestration;
...
- `TableWidget.vue` отвечает за DOM, Vue lifecycle, focus/edit orchestration;
...
3. `WidgetRenderer` публикует в subtree только допустимые table runtime services через definition-driven bridge;
4. таблица получает уже нормализованный `attrsByName`, error handlers и notifications только через injected/runtime boundary;
5. `TableWidget.vue` использует только этот слой, а не global/window или `$root`.

Для table-widget это означает:

- attrs map приходит только через `getAllAttrsMap`;
- recoverable app errors поднимаются через `handleRecoverableAppError`;
- user-facing notifications идут через `showAppNotification`;
- page-owned state таблица напрямую не читает и не мутирует.
...
Embedded cell widgets живут внутри table feature, но их runtime boundary теперь совпадает с общим widget contract:
...
- ad hoc доступ к page runtime через вложенные root-поля
